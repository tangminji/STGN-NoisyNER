...
import plotting_estonian_utils
from pylab import *
import pickle
...
f1_avgs, f1_err_bars = plotting_estonian_utils.compute_avg(ns_p_list)
with open(os.path.join(args.output_dir,'f1.txt'),'w') as f:
    for i in range(len(exp_dirs)):
        print(f'Exp {exp_dirs[i]}',file=f)
        for ni in f1_avgs[i]:
            print(f'{ni}: {f1_avgs[i][ni]:.4f}±{f1_err_bars[i][ni]:.4f}',file=f)
