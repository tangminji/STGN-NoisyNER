...
def preprocess():
    ...
    parser.add_argument('--input_dir', type=str, default="../data/estner_diff_noise/",
                        help='dir that contains the dataset')
    parser.add_argument('--output_dir', type=str, default="../preprocessed/",
                        help='dir that contains the processed data and model')
    ...
    for noisy_file_path in noisy_file_paths:
        ...
        save_root = os.path.join(args.output_dir, os.path.basename(os.path.dirname(noisy_file_path)))
    ...
    save_root = os.path.join(args.output_dir, os.path.basename(os.path.dirname(train_clean_path)))
    if not os.path.exists(save_root):
        os.makedirs(save_root)
...
def create_train_test_split(data_dir, outpur_dir):
    ...
    # 与NoisyNER的数据集划分保持一致
    train_length, dev_length = 185708, 208922-185708
...
def save_content_split(content, file_name, split_root):
    ...
    with open(file_path, 'w', encoding="utf-8") as file:
        for item in content:
            file.write("%s" % item)
            # file.write("%s\n" % item) # 这里其实最好不要加换行符,readlines中包含了换行符
