...
def train_others(args, model, loader, optimizer, criterion, global_iter, epoch, logpath):
    ...
    for i, (data, target, word, target_gt, index) in enumerate(loader):
        ...
        output = model(data)
        if (output.isnan()|output.isinf()).any():
            print('output',output)
            exit(1)
...
def main(params):
    ...
    if 'STGN' in args.exp_name:
        ...
        # 参数添加到路径中
        args.exp_name = os.path.join(args.exp_name, f'nr{args.noise_rate:.2f}_sigma{args.sigma}_times{args.times}')
...
if __name__ == '__main__':
    ...
    params = json.load(open(args.params_path, 'r', encoding="utf-8"))
    global ITERATION
