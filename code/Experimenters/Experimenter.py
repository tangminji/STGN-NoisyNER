...
class Experimenter():
    def __init__(self, all_datasets, label_representation, test_label_representation, embedding_vector_size,
        ...
        # # TODO 输出信息
        # self.f1_folder = os.path.join(self.exp_info_root,'f1')
        # utils.check_existence_and_create([self.f1_folder])
    ...
    # 计算噪声矩阵 根据干净标签、噪声标签 (直接对完整训练集计算，是"真实"转移矩阵)
    ...
    #         f1_file = os.path.join(self.f1_folder, f'{ns}.txt')
    #         with open(f1_file,"w") as f:
    #             for nt in range(self.num_times):
    #                 f1 = self.f1_hist[nt][ns]
    #                 f.write('%.2f' % f1+'\n')
    #                 f[i,nt] = f1
        
    #     f1_info = os.path.join(self.f1_folder,f'info.txt')
    #     with open(f1_info,"w") as f:
    #         for i,ns in enumerate(self.ns):
    #             f.write(f'ns{ns}:\tavg:{f[i].mean()}\n')
